...
const registerUserTs = async (basePath, noExperimentalResolver) => {
...
const loadModels = (modelsPaths) => {
    ...
    modelsPaths.forEach(async (singleModelPath) => {
        let exportedData;
        try {
            if (process.env.DEBUG) {
...
class MongooseTsgen extends Command {
    ...
    async generateDefinitions(customConfig) {
        ...
        const cleanupTs = await registerUserTs(flags.project, flags["no-experimental-resolver"]);
